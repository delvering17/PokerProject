...
public class Enter implements RoomAction{
	...
	@Override
	public void room(Login_frame login_frame,Receiver ch,Lobby lobby, MyData myData,Integer addr) {
		int chk =0;
		CHK:while(true) {
			if(myData.easyStudy[chk]>0 && myData.easyStudy[chk]<=5) {
//				tcpdata.DataDestination = "RoomChk";
//				tcpdata.easyStudy[chk]++;
//				tcpdata.UserPos = chk;
//				myData.DataDestination = "Chatting";
				login_frame.remove(login_frame.lobby_panel);
				Game_panel game_panel = new Game_panel(login_frame,ch,myData,chk);
